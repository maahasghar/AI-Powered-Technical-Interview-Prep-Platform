...
def test_progress_service_aggregates_rows_without_submissions():
    repository = type(
        "Repository", (), {"get_submission_rows": lambda self, user_id: []}
    )()
...
def test_progress_endpoint_is_user_scoped(
...
def test_account_deletion_removes_owned_data_but_keeps_problem(
    ...
    db_session.commit()
    db_session.add(
        Feedback(submission_id=submission.id, stage="DIAGNOSIS", status="FAILED")
    )
    db_session.commit()
    ...
    assert (
        db_session.query(Feedback).filter_by(submission_id=submission_id).count() == 0
    )
